...
int main(int argc, char **argv)
{
    ...
    cl_mem kernel_1D_d = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 255 * sizeof(float), kernel_1D, &clStatus);
    cl_mem kernel_2D_d = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, (w_size + 1) * (w_size + 1) * sizeof(float), kernel_1D, &clStatus);
	...
	clStatus = clSetKernelArg(kernel, 2, sizeof(cl_mem), &kernel_1D_d);
    clStatus = clSetKernelArg(kernel, 3, sizeof(cl_mem), &kernel_2D_d);
    clStatus = clSetKernelArg(kernel, 4, sizeof(cl_int), (void*)&w_size);
    clStatus = clSetKernelArg(kernel, 5, sizeof(cl_int), (void*)&width);
    clStatus = clSetKernelArg(kernel, 6, sizeof(cl_int), (void*)&height);
